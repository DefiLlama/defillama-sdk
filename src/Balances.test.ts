...
test("Balances - clone", async () => {
  ...
  balances.add("0001", [100, 200]);
  balances.add("tether", [100, 200], { skipChain: true });
  expect(balances.clone().getBalances()).toEqual({
    "bsc:0001": 300,
    tether: 300,
  });
  expect(balances.clone(0.5).getBalances()).toEqual({
    "bsc:0001": 150,
    tether: 150,
  });
});
...
test("Balances - add multiple tokens with label", async () => {
  const balances = new Balances({ chain: "bsc" });
  balances.addTokens(["tether", "usd-coin"], [100, 200], "stablecoins", {
    skipChain: true,
  });
...
test("Balances - addBalances with label and breakdown", async () => {
  ...
  expect(balances1.getBalances()).toEqual({
    tether: 150,
    "bsc:0001": 200,
    "bsc:0002": 300,
  });
  ...
  expect(breakdown["myTokens"].getBalances()).toEqual({
    "bsc:0001": 200,
    "bsc:0002": 300,
  });
});
...
test("Balances - tag + label combined", () => {
  ...
  expect(balances.getBreakdownBalances()["myTokens"].getBalances()).toEqual({
    "bsc:0001": 100,
  });
...
test("Balances - clone mirrors both breakdown and tags", async () => {
  ...
  expect(cloned.getBreakdownBalances()["myTokens"].getBalances()).toEqual({
    "bsc:0001": 100,
    "bsc:0002": 200,
  });
  expect(cloned.hasTaggedBalances()).toBe(true);
  expect(cloned.getTaggedBalances()["pool-A"].getBalances()).toEqual({
    "bsc:0001": 100,
  });
  expect(cloned.getTaggedBalances()["pool-B"].getBalances()).toEqual({
    "bsc:0002": 200,
  });
});
...
test("Balances - addBalances merges tags across instances", () => {
  ...
  expect(a.getTaggedBalances()["pool-A"].getBalances()).toEqual({
    "bsc:0001": 150,
  });
  expect(a.getTaggedBalances()["pool-B"].getBalances()).toEqual({
    "bsc:0002": 300,
  });
});
...
test("Balances - removeNegativeBalances cleans _balances, breakdown, and tagged", () => {
  ...
  expect(balances.getTaggedBalances()["pool-A"].getBalances()).toEqual({});
  expect(balances.getTaggedBalances()["pool-B"].getBalances()).toEqual({
    "bsc:0003": 200,
  });
});
...
test("Balances - debug", async () => {
  ...
  const {
    debugData: { tokenData },
  }: any = await balances.getUSDJSONs({
    debug: true,
    debugOptions: { printTokenTable: false },
  });
