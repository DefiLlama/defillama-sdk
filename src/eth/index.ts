...
export async function getBalances(params: {
	...
	try {
		const multicallContract = getMulticallAddress(
		...
		debugLog(
			"Multicall failed, falling back to single calls for fetching eth balances",
			e,
		);
	}
	...
	if (!output) {
		...
		if (params.permitFailure)
			output.forEach((o: any, idx: number) => {
				if (!o?.balance)
					output[idx] = { target: params.targets[idx], balance: "0" };
			});
	}
