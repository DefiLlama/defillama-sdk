...
function pickStableFields(x: any) {
  ...
  return {
    ...
    topicsLen:
      x.topics?.length ??
      [x.topic0, x.topic1, x.topic2, x.topic3].filter(Boolean).length,
...
function compareArgsStrict(
  ...
  const sortedA = Object.keys(argsA || {})
    .sort()
  ...
  const sortedB = Object.keys(argsB || {})
    .sort()
    .reduce((acc: any, key: string) => {
      acc[key] = argsB[key];
      return acc;
  ...
  if (jsonA !== jsonB) {
      ...
      if (!hasA) {
        ...
        if (valA !== valB) {
          differentValues.push(
            `${key}: A=${valA.substring(0, 100)}, B=${valB.substring(0, 100)}`,
          );
        }
      }
    }
    ...
    if (missingInB.length > 0) {
      differences.push(
        `  Keys present in A but not in B: ${missingInB.join(", ")}`,
      );
    }
    if (missingInA.length > 0) {
      differences.push(
        `  Keys present in B but not in A: ${missingInA.join(", ")}`,
      );
    }
...
function findCommonTransaction(
  results: TestResult[],
): { txHash: string; logIndex: number; log: any } | null {
  ...
  for (const candidateLog of firstResult.logs) {
    const candidateTxHash =
      candidateLog.transactionHash ??
      candidateLog._originalTransactionHash ??
      candidateLog.transaction_hash;
    const candidateLogIndex =
      candidateLog.logIndex ?? candidateLog.index ?? candidateLog.log_index;
...
async function runTestConfig(config: TestConfig): Promise<TestResult> {
  ...
  const processor = async (logs: any[]) => {
    ...
    if (config.clientStreaming && chunkLogs >= 100000) {
      ...
      metrics.chunks.push({
        ...
        logsInChunk: chunkLogs,
        bytesInChunk: bytes,
        timeForChunk: chunkTime,
        mbps,
      });
  ...
  const logs = await getLogs({
    ...
    processor,
    collect: true,
    onWireStats: config.clientStreaming
      ? ({ bytesReceived }: any) => {
  ...
  if (config.clientStreaming) {
    ...
    if (metrics.chunks.length > 0) {
      metrics.avgMbps =
        metrics.chunks.reduce((s, c) => s + c.mbps, 0) / metrics.chunks.length;
    }
  }

  metrics.logsPerSecond = logs.length / (metrics.totalTime / 1000);
  if (metrics.decodeBatches > 0) {
    metrics.avgDecodeTimePerBatch =
      metrics.totalDecodeTime / metrics.decodeBatches;
    metrics.decodeTimePerLog =
      metrics.totalDecodeTime / Math.max(metrics.totalLogs, 1);
  }
  ...
  test("Compare all combinations: streaming/legacy × viem/ethers", async () => {
    ...
    if (testConfigIndicesEnv) {
      ...
      if (indices.length > 0) {
        ...
        console.log(
          `🧪 Testing only configurations at indices: ${indices.join(", ")}`,
        );
        console.log(
          `   Configurations: ${indices.map((idx) => getConfigLabel(allConfigs[idx])).join(", ")}`,
        );
      } else {
        console.warn(
          `⚠️  Invalid TEST_CONFIG_INDICES="${testConfigIndicesEnv}", testing all configurations`,
        );
      }
    }
    ...
    results.forEach((result) => {
      ...
      reportLines.push(
        `  📈 Total: ${m.totalLogs.toLocaleString()} logs in ${formatDuration(m.totalTime)}`,
      );
      reportLines.push(`  ⚡ TTFB: ${formatDuration(m.ttfb)}`);
      if (result.config.clientStreaming) {
        reportLines.push(
          `  📦 Total size (wire): ${formatBytes(m.totalBytes)}`,
        );
        reportLines.push(
          `  🚀 Average throughput: ${m.avgMbps.toFixed(2)} MB/s`,
        );
      }
      ...
      if (m.decodeBatches > 0) {
        reportLines.push(
          `  🔧 Decode time: ${formatDuration(m.totalDecodeTime)} (${m.decodeBatches} batches)`,
        );
        reportLines.push(
          `  🔧 Avg decode time/batch: ${formatDuration(m.avgDecodeTimePerBatch)}`,
        );
        reportLines.push(
          `  🔧 Decode time/log: ${m.decodeTimePerLog.toFixed(3)}ms`,
        );
        const decodePercentage = (m.totalDecodeTime / m.totalTime) * 100;
        reportLines.push(
          `  🔧 Decode % of total time: ${decodePercentage.toFixed(2)}%`,
        );
      }
    });
    ...
    results.forEach((r) => {
      ...
      const decodePct =
        r.metrics.totalTime > 0
          ? ((r.metrics.totalDecodeTime / r.metrics.totalTime) * 100).toFixed(
              1,
            ) + "%"
          : "0%";
      reportLines.push(
    ...
    const streamingViem = results.find(
      (r) => r.config.clientStreaming && r.config.decoderType === "viem",
    );
    const streamingEthers = results.find(
      (r) => r.config.clientStreaming && r.config.decoderType === "ethers",
    );

    if (streamingViem && streamingEthers) {
      const timeImprovement =
        ((streamingEthers.metrics.totalTime - streamingViem.metrics.totalTime) /
          streamingEthers.metrics.totalTime) *
        100;
      const decodeImprovement =
        ((streamingEthers.metrics.totalDecodeTime -
          streamingViem.metrics.totalDecodeTime) /
          streamingEthers.metrics.totalDecodeTime) *
        100;
      reportLines.push(`\nStreaming: Viem vs Ethers`);
      reportLines.push(
        `  Total time improvement: ${timeImprovement > 0 ? "+" : ""}${timeImprovement.toFixed(2)}%`,
      );
      reportLines.push(
        `  Decode time improvement: ${decodeImprovement > 0 ? "+" : ""}${decodeImprovement.toFixed(2)}%`,
      );
    }

    const legacyViem = results.find(
      (r) => !r.config.clientStreaming && r.config.decoderType === "viem",
    );
    const legacyEthers = results.find(
      (r) => !r.config.clientStreaming && r.config.decoderType === "ethers",
    );

    if (legacyViem && legacyEthers) {
      const timeImprovement =
        ((legacyEthers.metrics.totalTime - legacyViem.metrics.totalTime) /
          legacyEthers.metrics.totalTime) *
        100;
      const decodeImprovement =
        ((legacyEthers.metrics.totalDecodeTime -
          legacyViem.metrics.totalDecodeTime) /
          legacyEthers.metrics.totalDecodeTime) *
        100;
    ...
    reportLines.push("\n" + "=".repeat(80));
    reportLines.push(
      "🔍 STRICT ARGS VALIDATION - Same Transaction Across All Configs",
    );
    reportLines.push("=".repeat(80));
    ...
    if (commonTx) {
      ...
      if (txLogs.length === results.length) {
        ...
        if (refLog.args) {
          const viemExample = txLogs.find(
            (tx) => tx.config.decoderType === "viem",
          );
          const ethersExample = txLogs.find(
            (tx) => tx.config.decoderType === "ethers",
          );
          ...
          if (viemExample && viemExample.log.args) {
            ...
            const viemArgsDisplay = JSON.stringify(
              viemExample.log.args,
              (key, value) => {
            ...
            if (viemArgsDisplay.length > 2000) {
              reportLines.push(
                viemArgsDisplay.substring(0, 2000) + "\n  ... (truncated)",
              );
            } else {
              reportLines.push(
                viemArgsDisplay
          ...
          if (ethersExample && ethersExample.log.args) {
            ...
            const ethersArgsDisplay = JSON.stringify(
              ethersExample.log.args,
              (key, value) => {
                if (typeof value === "bigint") {
                  return value.toString();
                }
            ...
            if (ethersArgsDisplay.length > 2000) {
              reportLines.push(
                ethersArgsDisplay.substring(0, 2000) + "\n  ... (truncated)",
              );
            } else {
              reportLines.push(
                ethersArgsDisplay
          ...
          for (let i = 1; i < txLogs.length; i++) {
            ...
            const comparison = compareArgsStrict(
              refLog.args,
              current.log.args,
              `${refConfig} vs ${currentConfig}`,
            );
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      if (current.logs.length > 0 && firstResult.logs.length > 0) {
        ...
        const sampleIndices = [
          0,
          Math.floor(firstResult.logs.length / 2),
          firstResult.logs.length - 1,
        ].filter((idx) => idx < firstResult.logs.length);
