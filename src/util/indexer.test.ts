...
test("Indexer - getTokenTransfers", async () => {
  ...
  const res = await getTokenTransfers({
    ...
    tokens: [
      ...
      "0x09faeb69e29845f3326e4f004f45a31ceb0eedb9",
    ],
    fromBlock: 119877801,
    toBlock: 119943935,
    chain: "arbitrum",
...
test("Indexer - getLogs - noTarget with large block range should throw", async () => {
  ...
  await expect(res).rejects.toThrow(
    "When noTarget is true, block range must be less than 500k blocks",
  );
});
...
test("Indexer - getLogs with processor", async () => {
  ...
  expect(balances["arbitrum:0x999FAF0AF2fF109938eeFE6A7BF91CA56f0D07e1"]).toBe(
    "225237781369731800000",
  );
  expect(balances["arbitrum:0x577Fd586c9E6BA7f2E85E025D5824DBE19896656"]).toBe(
    "1.2409671794946094e+22",
  );
  expect(balances["arbitrum:0x4e6b45BB1C7D11402faf72c2d59cAbC4085E36f2"]).toBe(
    "2.0821579300721433e+27",
  );
  expect(balances["arbitrum:0xe47ba52f326806559c1deC7ddd997F6957d0317D"]).toBe(
    "574795991880981700000",
  );
  expect(balances["arbitrum:0x83e5Ecd192eAc043B0674A16EEDf96176726A159"]).toBe(
    "9.92413672876591e+22",
  );
  expect(balances["arbitrum:0xA533f744B179F2431f5395978e391107DC76e103"]).toBe(
    "272211934709000000000",
  );
  expect(balances["arbitrum:0x4F604735c1cF31399C6E711D5962b2B3E0225AD3"]).toBe(
    "1e+21",
  );
  expect(balances["arbitrum:0xC760F9782F8ceA5B06D862574464729537159966"]).toBe(
    "2.9174585867738748e+22",
  );
  expect(balances["arbitrum:0x66E535e8D2ebf13F49F3D49e5c50395a97C137b1"]).toBe(
    "3768845891207509000",
  );
  expect(balances["arbitrum:0x3269a3C00AB86c753856fD135d97b87FACB0d848"]).toBe(
    "1.2449725048906123e+22",
  );
  expect(balances["arbitrum:0xC3323b6e71925b25943fB7369EE6769837e9C676"]).toBe(
    "8.9999999e+21",
  );
  expect(balances["arbitrum:0x0721b3C9f19cfeF1d622C918DcD431960f35E060"]).toBe(
    "2.2350494277261517e+22",
  );
});

test("Indexer - getTransactions", async () => {
  const txHash =
    "0x1d1a14b882adf9d9c078a9868b682eba7833ebfd59ee0a93aa477c990056aa79";
...
test("Indexer - getLogs - Viem vs Ethers comparison", async () => {
  ...
  for (let i = 0; i < viemLogs.length; i++) {
    ...
    expect(viemLog.logIndex ?? viemLog.index).toBe(
      ethersLog.logIndex ?? ethersLog.index,
    );
