...
import {
	getParallelGetBlocksLimit,
	ENV_CONSTANTS,
	shouldSkipBlockValidation,
} from "./env";
...
export async function getBlock(
	chain: Chain,
	timestamp?: number,
	options: LookupBlockOptionalParams = {},
): Promise<Block> {
...
const algorandBlockProvider = {
	getBlock: async (height: number | "latest"): Promise<TimestampBlock> => {
		if (height !== "latest")
			return fetchJson(
				`https://algoindexer.algoexplorerapi.io/v2/blocks/${height}`,
			).then((block: any) => ({
				number: block.round,
				timestamp: block.timestamp,
			}));
		return fetchJson("https://algoindexer.algoexplorerapi.io/health").then(
			(block: any) => algorandBlockProvider.getBlock(block.round),
		);
	},
};
...
function getExtraProvider(chain = "ethereum") {
	if (isCosmosChain(chain))
		// maybe check if it is also an evm chain?
...
const {
	data: blockTimeCache,
	saveCacheFile: saveBlockCacheFile,
}: { data: ChainBlockCache; saveCacheFile: Function } = getTempLocalCache({
	file: "BlockCache.json",
	defaultData: {},
	clearAfter: ONE_WEEK,
	returnWithSaveFunction: true,
});
...
async function _lookupBlock(
	...
	if (timestamp > time / 1000) {
		throw new Error(
			`Requesting for block in the future ${timestamp} is in the future, current time is ${Math.floor(time / 1000)}`,
		);
	}
	...
	if (envLowValue) low = parseInt(envLowValue);
	...
	try {
		...
		if (["evmos", "nibiru"].includes(chain)) {
			...
			switch (chain) {
				case "nibiru":
					firstBlockNum -= 4 * 1e5; // nibiru hold only the last 400k block data
				case "evmos":
