...
import {
  createViemFastPathBatchDecoder,
  normalizeLog,
} from "./logs.decode.shared";
import { GetTransactionOptions } from "./transactions";

const LLAMA_INDEXER_ENDPOINT = getEnvValue("LLAMA_INDEXER_V2_ENDPOINT");
const LLAMA_INDEXER_API_KEY = getEnvValue("LLAMA_INDEXER_V2_API_KEY");
const addressChunkSize =
  +getEnvValue("LLAMA_INDEXER_ADDRESS_CHUNK_SIZE")! || 100;
...
function checkIndexerConfig() {
  if (!LLAMA_INDEXER_ENDPOINT || !LLAMA_INDEXER_API_KEY)
    throw new Error(`Llama Indexer URL/api key is not set`);
}

function getChainId(chain: string): number {
  const chainId = Object.entries(indexerChainIdChainMapping).find(
...
function getSupportedChains(): Set<string> {
  return new Set(Object.values(indexerChainIdChainMapping));
}

export const supportedChainSet2 = getSupportedChains();

type ChainIndexStatus = {
  [chain: string]: { block: number; timestamp: number };
};
const state: {
  timestamp?: number;
...
async function getChainIndexStatus(): Promise<ChainIndexStatus> {
  checkIndexerConfig();
...
export async function getLogs(options: IndexerGetLogsOptions): Promise<any[]> {
  ...
  if (processor && targets?.length > 1 && !flatten)
    throw new Error(
      "processor is not supported with multiple targets when flatten=false",
    );

  const blockRange = toBlock - fromBlock;
  const effectiveMaxBlockRange =
    maxBlockRange ?? (noTarget ? 10_000 : Infinity);
  ...
  if (lastIndexedBlock < toBlock) {
    const percentageMissing =
      ((toBlock - lastIndexedBlock) / (toBlock - fromBlock)) * 100;
    ...
    if (ENV_CONSTANTS.GET_LOGS_INDEXER)
      debugLog(
        `Indexer only partially up to date for ${chain}. Last indexed block: ${lastIndexedBlock}, requested block: ${toBlock}, missing ${Number(percentageMissing).toFixed(2)}%. Pulling part of the logs through RPC calls.`,
      );

    const breakBlock = lastIndexedBlock - 50; // small buffer
    const indexerLogs = await getLogs({
      ...options,
      fromBlock,
      toBlock: breakBlock,
    });
    const rpcLogs = await getLogsParent({
      ...options,
      fromBlock: breakBlock + 1,
      toBlock,
      skipIndexer: true,
    });
  ...
  if (blockRange > effectiveMaxBlockRange) {
    ...
    for (
      ...
      const currentToBlock = Math.min(
        currentFromBlock + effectiveMaxBlockRange - 1,
        toBlock,
      );
  ...
  const addressChunks = sliceIntoChunks(
    address?.split(",") ?? [],
    addressChunkSize,
  );
  if (noTarget && addressChunks.length === 0)
    addressChunks.push(undefined as any);
  ...
  if (clientStreaming) {
    ...
    let remainingLimit = shouldLimit
    ...
    const flushBatch = async (batch: any[]): Promise<void> => {
      ...
      if (batchFn) {
        ...
        for (let i = 0; i < batch.length; i++) {
          if (options.onlyArgs) {
            ...
            if (
              !transformedLogs[i].transactionHash &&
              transformedLogs[i]._originalTransactionHash
            ) {
              transformedLogs[i].transactionHash =
                transformedLogs[i]._originalTransactionHash;
            }
        ...
        transformedLogs = await Promise.all(
          batch.map((log: any) => transformLog(log)),
        );
      }
      ...
      if (splitByAddress) {
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          const idx =
            addressIndexMap[(raw.source ?? raw.address)?.toLowerCase?.()];
          if (idx !== undefined) {
    ...
    for (const chunk of addressChunks) {
      ...
      if (chunk && Array.isArray(chunk))
        qs.set("addresses", chunk.join(",").toLowerCase());
      ...
      await streamJsonArrayFromIndexer({
        ...
        onItem: (raw) => {
          ...
          const okAddress =
            !addressSet.size ||
            addressSet.has((raw.source ?? raw.address)?.toLowerCase?.());
          if (!okAddress) return;
    ...
    if (debugMode) {
      ...
      debugLog(
        `[Indexer] stream finished: ${outPairs.length} items in ${ms}ms`,
      );
    }
  ...
  for (const chunk of addressChunks) {
    ...
    do {
      ...
      const { logs: _logs, totalCount } = await indexerFetch(
        `/logs`,
        params,
      ).catch((e) => {
        throw formError(e);
      ...
      const filtered = _logs.filter((l: any) => {
        const isWhitelisted =
          !addressSet.size ||
          addressSet.has((l.source ?? l.address)?.toLowerCase?.());
      ...
      const transformBatchFn =
        (transformLog as any).batch ||
        ((logs: any[]) =>
          Promise.all(logs.map((log: any) => transformLog(log))));

      const transformedLogs = await transformBatchFn(filtered);

      const transformedPair = filtered.map((log: any, idx: number) => ({
        raw: log,
        transformed: transformedLogs[idx],
      }));
      const decodeTime = Date.now() - t0;
      if (onDecodeStats && filtered.length > 0)
        onDecodeStats({
          batchSize: filtered.length,
          decodeTime,
          itemsDecoded: filtered.length,
        });

      if (processor)
        await processor(transformedPair.map((i: any) => i.transformed));
...
export async function getTokenTransfers({
  ...
  if (typeof fromAddressFilter === "string")
    fromAddressFilter = [fromAddressFilter];
  const fromFilterSet = new Set(
    (fromAddressFilter ?? []).map((a) => a.toLowerCase()),
  );
  ...
  if (tokens) {
    ...
    if (!Array.isArray(tokens))
      throw new Error("tokens must be a string or an array of strings");
  ...
  do {
    ...
    const { transfers: _logs } = await indexerFetch(
      `/token-transfers`,
      params,
    ).catch((e) => {
      throw formError(e);
...
export async function getTransactions({
  ...
  if (
    ...
    (!transaction_hashes || transaction_hashes.length === 0)
  )
    throw new Error(
      "You must provide at least 'addresses' or 'transaction_hashes'",
    );
  if (!from_block || !to_block)
    throw new Error(
      "'from_block' and 'to_block' are required to search for transactions",
    );
  ...
  const { transactions } = await indexerFetch(`/transactions`, params).catch(
    (e) => {
      throw formError(e);
    },
  );
  ...
  if (debugMode) {
    console.timeEnd(debugTimeKey);
    debugLog(
      "Transactions pulled " + chain,
      addresses || transaction_hashes,
...
export function isIndexerEnabled(chain?: string) {
  if (!LLAMA_INDEXER_ENDPOINT) return false;
