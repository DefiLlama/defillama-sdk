...
import {
  ...
  chainLabelsToKeyMap,
} from "../util/chainUtils";

const concurrentCheckChains = +(process.env.SDK_BUILD_CONCURRENT_CHAINS || 7);
const chainRemovalThreshold = +(
  process.env.SDK_BUILD_CHAIN_REMOVAL_THRESHOLD || 20
);
...
async function main() {
  const chainKeyLabelMapCountBefore = Object.keys(
    chainKeyToChainLabelMap,
  ).length;
  const chainLabelsKeyMapCountBefore = Object.keys(chainLabelsToKeyMap).length;
  await updateData();

  const chainKeyLabelMapCountAfter = Object.keys(
    chainKeyToChainLabelMap,
  ).length;
  ...
  if (chainKeyLabelMapCountAfter > chainKeyLabelMapCountBefore) {
    console.log(
      `Updated chainKeyToChainLabelMap: ${chainKeyLabelMapCountBefore} -> ${chainKeyLabelMapCountAfter}`,
    );
  }
  if (chainLabelsKeyMapCountAfter > chainLabelsKeyMapCountBefore) {
    console.log(
      `Updated chainLabelsToKeyMap: ${chainLabelsKeyMapCountBefore} -> ${chainLabelsKeyMapCountAfter}`,
    );
  }

  if (chainKeyLabelMapCountAfter < chainKeyLabelMapCountBefore)
    throw new Error(
      "chainKeyToChainLabelMap count decreased, please investigate",
    );
  if (chainLabelsKeyMapCountAfter < chainLabelsKeyMapCountBefore)
    throw new Error("chainLabelsToKeyMap count decreased, please investigate");

  const oldProviders = await fetchJson(
    `https://unpkg.com/@defillama/sdk@latest/build/providers.json`,
  );
  const currentChains = await fetchJson(
    `https://raw.githubusercontent.com/DefiLlama/DefiLlama-Adapters/refs/heads/main/projects/helper/chains.json`,
  );
  ...
  chainData = chainData
    ...
    .filter((i: any) => {
      ...
      const isTestnetKey = bannedKeys.some((j: string) =>
        i.shortName.includes(j),
      );
      const hasTestnetRPC = i.rpc.some((rpc: any) =>
        bannedRPCKeys.some((k: string) => rpc?.url.includes(k)),
      );
      const hasTestnetName =
        i.name?.toLowerCase().includes("testnet") ||
        i.name?.toLowerCase().includes("devnet");
  ...
  const rpcFilterRegex =
    /(wss\:|ws\:|terminet\.io|huobichain\.com|getblock\.io|bitstack\.com|nodereal.io\/v1|pokt\.network|histori\.xyz|chainstacklabs\.com|owlracle\.info|blastapi\.io)/;
  ...
  await PromisePool.withConcurrency(concurrentCheckChains)
    .for(chainData)
    .process(async (i: any) => {
      ...
      if (providerIDMap[i.chainId]) {
        ...
        if (!isBEVM) providerIDMap[i.chainId].push(...i.rpc);
  ...
  const droppedChains = Object.keys(oldProviders).filter(
    (oldChain) => providerList[oldChain] === undefined,
  );
  const filteredDroppedChains = droppedChains.filter(
    (i) => !droppedTestnetChainsSet.has(i) && !i.includes("test"),
  );
  console.log("Dropped chains before testnet filtering:", droppedChains.length);
  console.log(
    "Dropped chains after testnet filtering:",
    filteredDroppedChains.length,
  );
  if (filteredDroppedChains.length > chainRemovalThreshold) {
    throw new Error(
      `Following chains used to be included but is not anymore, can the devs fix please?\n${filteredDroppedChains.join(", ")}`,
    );
  }

  delete providerList.bitcoin; // what were they smoking?

  const rpcCount = Object.values(providerList).reduce(
    (acc, i) => acc + (i?.rpc.length ?? 0),
    0,
  );
  const rpcCountOld = Object.values(oldProviders).reduce(
    (acc: any, i: any) => acc + (i?.rpc.length ?? 0),
    0,
  );
  const newlyAddedChains = Object.keys(providerList).filter(
    (chain) => oldProviders[chain] === undefined,
  );
...
const blacklist: string[] = ["https://eth.api.onfinality.io/public"];
...
function filterRPCs(rpc: string[]): string[] {
  return rpc
    .filter((i: string) => {
      ...
      if (
        /(wss\:|ws\:|http\:|test|devnet|terminet\.io|huobichain\.com|getblock\.io|bitstack\.com|nodereal.io\/v1|pokt\.network|ankr\.com|histori\.xyz|chainstacklabs\.com|owlracle\.info|blastapi\.io|\=)/.test(
          i,
        )
      )
        return false; // remove anything with blacklisted words
      ...
      try {
        ...
        if (data.result) {
          ...
          if (returnedChainId !== chainId) {
            console.log(
              `RPC ${i} for ${chain} returned invalid chainId ${returnedChainId}, expected ${chainId}`,
            );
          }
          filteredRPCs.push(i);
        }
