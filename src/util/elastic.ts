...
export async function writeLog(
  ...
  if (!log.hasOwnProperty("timestamp")) log.timestamp = +Date.now();
...
export async function addDebugLog(body: { data: object; metadata: Metadata }) {
...
export async function addErrorLog(body: { error: object; metadata: Metadata }) {
...
export async function search(
  ...options: Parameters<Client["search"]>
): Promise<ReturnType<Client["search"]>> {
  const client = getClient();
...
export async function getAllLogs({
  ...
  while (response.hits.hits.length) {
    ...
    debugLog(
      `Fetched ${allLogs.length} records, ${response.hits.hits.length} in batch`,
    );
