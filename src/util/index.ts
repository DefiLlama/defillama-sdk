...
export function tableToString(
  data: any,
  columns?: any,
  { title, columns: _columns }: { title?: string; columns?: string[] } = {},
) {
  ...
  if (_columns && Array.isArray(_columns)) columns = _columns;
  ...
  const columnWidths = columns.map((col: any) =>
    Math.max(
      col.length,
      ...data.map((row: any) =>
        row[col] !== undefined ? String(row[col]).length : 0,
      ),
    ),
  );
