...
export function sumSingleBalance(
  ...
  if (typeof balance === "object") {
    ...
    else throw new Error("Invalid balance value:" + balance + " " + token);
  }
  if (!token || token.trim() === "") {
...
export function formErrorString(e: any, errorParams: any = {}) {
  if (e?.config?.url || e?.response?.status) {
    ...
    if (e?.response?.data) {
      ...
      if (
        dataString.startsWith('"<!DOCTYPE html>') ||
        dataString.startsWith('"<html')
      )
        dataString = "HTML error page";
      message += dataString;
      return message;
    }

    if (e?.message) return shortenString(message + e.message);
  }

  const isGraphError =
    typeof e === "object" &&
    e?.stack?.includes("graphql-request") &&
    e.response?.errors?.length;

  if (isGraphError)
    return shortenString(
      `GraphQL request failed! ${e.response.errors.map((i: any) => i.message).join(", ")}`,
    );
  ...
  if (e.llamaRPCError) {
    ...
    const errors = e.errors.map(
      (i: any) =>
        `- host: ${i?.host} error: ${typeof i?.error === "string" ? i?.error : i?.error?.message}`,
    );
    return errorString + "\n" + errors.join("\n");
  }
...
export function shortenString(str: string, length?: number) {
  if (!length) {
    if (!isNaN(+ENV_CONSTANTS.DEFAULT_SHORTEN_STRING_LENGTH!))
      length = +ENV_CONSTANTS.DEFAULT_SHORTEN_STRING_LENGTH!;
...
export async function postJson(url: string, data: any = {}, options: any = {}) {
...
export function fixTronCallParams(params: any) {
  ...
  if (typeof params === "object") {
    ...
    if (Array.isArray(params.params))
      params.params = params.params.map(wrappedHexify);
    if (typeof params.params === "string")
      params.params = wrappedHexify(params.params);
  }

  if (Array.isArray(params.calls)) params.calls.forEach(fixTronCallParams);
}
