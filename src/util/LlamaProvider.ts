...
export class LlamaProvider extends FallbackProvider {
    ...
    async function checkRPCWorks({ url, provider }: ProviderWithURL) {
      try {
        ...
        if (ENV_CONSTANTS.LOG_LLAMA_PROVIDER_CALLS)
          debugLog(
            `${_this.chainName} skipping RPC ${url} is not working, error: ${(e as any).message}`,
          );
  ...
  async _performAction(
    ...
    if (!rpcRequestCounter[this.chainName])
      rpcRequestCounter[this.chainName] = 0;
    ...
    } else runners = runners.slice(1).sort(() => Math.random() - 0.5); // randomize order of runners
...
export function getProvider(
  ...
  if (!providers[chainKey])
    providers[chainKey] = createProvider(
      chain,
      rpcList,
...
const httpRPC = {
  ...
  getBlock: async (rpc: string, params: any): Promise<any> => {
    ...
    const { result, error } = await rpcFetch(
      ...
      +(getEnvValue("LLAMA_PROVIDER_RPC_GET_BLOCK_TIMEOUT", "60000") as any),
    );
  ...
  getBalance: async (rpc: string, params: any): Promise<any> => {
    ...
    const { result, error } = await rpcFetch(
      ...
      +(getEnvValue("LLAMA_PROVIDER_RPC_GET_BALANCE_TIMEOUT", "30000") as any),
    );
    if (error) throw error;
  ...
  call: async (rpc: string, params: any): Promise<any> => {
    ...
    const data = await rpcFetch(
      ...
      +(getEnvValue("LLAMA_PROVIDER_RPC_CALL_TIMEOUT", "30000") as any),
    );
    if (data.error) throw data.error;
    return data.result;
  },
  getLogs: async (rpc: string, params: any): Promise<any> => {
    ...
    const { result, error } = await rpcFetch(
      ...
      +(getEnvValue("LLAMA_PROVIDER_RPC_GET_LOGS_TIMEOUT", "30000") as any),
    );
    if (error) throw error;
...
function printRPCCallStats() {
  const entries = Object.entries(rpcRequestCounter)
