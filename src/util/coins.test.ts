...
test("coinsApi - mcaps", async () => {
	...
	expect(res["coingecko:tether"].timestamp).toBeGreaterThan(
...
test("coinsApi - prices", async () => {
	...
	expect(
		res["ethereum:0xDAc17f958d2ee523a2206206994597c13d831ec7"].symbol,
	).toBe("USDT");
	expect(
		res2["ethereum:0xDAc17f958d2ee523a2206206994597c13d831ec7"].decimals,
	).toBe(6);
	expect(
		res2["ethereum:0xdac17f958d2ee523a2206206994597c13d831ec7"].decimals,
	).toBe(6);
	...
	expect(
		res["solana:So11111111111111111111111111111111111111112"].timestamp,
	).toBeGreaterThan(Math.floor(Date.now() / 1e3 - 3600));
});
...
test("coinsApi - prices with timestamp", async () => {
	...
	expect(res["coingecko:ethereum"].timestamp).toBeLessThanOrEqual(
		timestamp - 7200 + 3600,
	); // within an hour of requested timestamp
	expect(Object.keys(res).length).toBe(1);
});
