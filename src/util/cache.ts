...
function unzipAsync(data: Buffer) {
	return promisify(brotliDecompress)(data as any, brotliOptions).then((r) =>
		r.toString(),
	);
}
...
export async function readCache(
	...
	if (options.skipCompression) file = `${file}-uncompressed`;
...
export async function writeCache(
	...
	try {
		const fileData = options.alreadyCompressed
			? data
			: await compressCache(data, options);
...
export async function parseCache(
	...
	if (typeof dataString === "string")
		dataBuffer = Buffer.from(dataString, "base64");
	else {
	...
	let _unzipAsync = (str: Buffer) =>
		options.skipCompression ? dataString : unzipAsync(str); // if skipCompression is true, we don't decompress the data
...
export async function compressCache(
	...
	if (options.skipCompression) return dataString;
...
export function getTempLocalCache({
	...
	defaultData = {},
	clearAfter = ONE_WEEK,
	returnWithSaveFunction = false,
}: {
	file: string;
	defaultData?: any;
	clearAfter?: number;
	returnWithSaveFunction?: boolean;
}): any | { data: any; saveCacheFile: () => void } {
	...
	function saveCache(skipCacheFlagCheck = false) {
		...
		try {
			...
			try {
				debugLog("Error saving cache:", error?.toString());
			} catch (error) {}
		}
	}
...
export async function cachedFetch({
	...
	if (!key) {
		if (!endpoint || postData || fetcher)
			throw new Error("Key is required for cachedFetch");
	...
	if (cacheMaxAge > 0) {
		try {
			const filePath = getFilePath(
				readCacheOptions.skipCompression ? `${fileKey}-uncompressed` : fileKey,
			);
			const stats = await fs.stat(filePath);
	...
	try {
		...
		if (fetcher) data = await fetcher();
			...
			if (!endpoint)
				throw new Error(
					"Either fetcher or endpoint is required for cachedFetch",
				);
