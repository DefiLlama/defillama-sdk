...
function getLink(project: string, chain: string) {
...
async function _setCache(project: string, chain: string, cache: any) {
  const Key = getKey(project, chain)

  try {
    await writeCache(getFileKey(project, chain), cache)
  } catch (e) {
    log('failed to write data to s3 bucket: ', Key)
    log(e)
  }
}

export const configCache: { [key: string]: any } = {}
...
export async function setCache(project: string, chain: string, json: any) {
  ...
  if (isValidData) // sometimes we get bad data/empty object, we dont overwrite cache with it
    await _setCache(project, chain, json)
}

export async function getConfig(project: string, endpoint: string, { fetcher } = { fetcher: () => Promise<any> }) {
  resetCache()
  ...
  async function _getConfig() {
    try {
      ...
      // log(e)
      log(project, 'tryng to fetch from cache, failed to fetch data from endpoint:', endpoint)
