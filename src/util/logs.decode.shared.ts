...
export function convertTopicsToViemFormat(
	topics: any[],
): [`0x${string}`, ...(`0x${string}`[] | [])] | [] {
	...
	return filteredTopics.length > 0
		? ([filteredTopics[0], ...filteredTopics.slice(1)] as [
				`0x${string}`,
				...(`0x${string}`[] | []),
			])
		: ([] as []);
...
export function enrichEthersArgsWithNamedProperties(
	...
	const length =
		typeof ethersArgs.length === "number" ? ethersArgs.length : inputs.length;
	...
	try {
		...
		if (typeof decodedArgs === "object") {
			...
			for (let i = 0; i < inputCount; i++) {
				const input = eventInputs[i];
				const value =
					(decodedArgs as any)[input.name] ?? (decodedArgs as any)[i] ?? null;
				if (value !== undefined && value !== null) {
					const normalizedValue = normalizeFn(value, input.type);
					if (input.name) args[input.name] = normalizedValue;
					args[i] = normalizedValue;
...
export function createViemFastPathBatchDecoder(
	eventAbi: string | any,
): ((logs: any[]) => Promise<any[]>) | null {
	try {
		// Parse ABI once
		...
		return async function decodeBatchViemFast(logs: any[]): Promise<any[]> {
			...
			for (let i = 0; i < logs.length; i++) {
				...
				try {
					...
					const decodedNon =
						nonIndexed.length && logData && logData !== "0x"
							? decodeAbiParameters(
									nonIndexedTypes as any,
									logData as `0x${string}`,
								)
							: [];
					...
					for (let j = 0; j < inputs.length; j++) {
						...
						if (value !== null && value !== undefined) {
							...
							if (
								...
								try {
									normalizedValue = ethers.getAddress(
										addressStr,
									) as `0x${string}`;
								} catch {
