...
import {
  enrichEthersArgsWithNamedProperties,
  padTopic32,
...
export async function getLogs(
  ...
  if (firstRun || skipCacheRead) {
    ...
    for (const c of caches) {
      ...
      if (
        _to < firstCache.metadata.fromBlock ||
        _from > lastCache.metadata.toBlock
      ) {
  ...
  async function addLogsToCache(fromBlock: number, toBlock: number) {
    ...
    if (!skipCache) {
      ...
      if (fileSizeInMB > 300) {
        // if cache size is larger than ~300MB, trim it down
        console.log(
          `getLogs: large cache size detected ${totalLogCount} logs size: ${fileSizeInMB}MB, retaining only the latest 200k logs to limit memory usage target=${target} topic=${topic} fromBlock=${fromBlock} toBlock=${toBlock}`,
        );
        ...
        if (caches.length > 1) {
          ...
          for (const c of caches) {
            if (
              c.metadata.fromBlock > cacheWithLatestFromBlock.metadata.fromBlock
            ) {
      ...
      await writeCache(
        getFile(),
        { caches: storeCaches, version: currentVersion },
        { skipR2CacheWrite: !cacheInCloud, skipCompression: !cacheInCloud },
      );
    }
  }
  ...
  async function _getCache(): Promise<logCache[]> {
    ...
    let cache = await readCache(key, {
      skipR2Cache: !cacheInCloud,
      skipCompression: !cacheInCloud,
    });
  ...
  function getFile() {
    ...
    const normalKeyLength =
      "0x27e5cb712334e101b3c232eb0be198baaa595f5f-0xe8137aa901976cc8eaf1cef5dec491873faadc99d9720ccaec95673294a9d7c5-uncompressed"
        .length;

    // sometimes, there is a lot of topics and we hit the file name length limit
    if (file.length > normalKeyLength) file = getHash(file);
...
export async function getLogParams(
  ...
  if (!topics?.length) {
    ...
    else throw new Error("eventAbi | topic | topics are required");
    topics = [topic];
  ...
  function transformLog(log: any) {
    ...
    if (isIndexerCall) {
      ...
      ["chain", "log_index"].forEach((k) => delete log[k]);
    }
    ...
    if (!log.topics && log._originalTopics) {
      log.topics = [
        log._originalTopics.topic0,
        log._originalTopics.topic1,
        log._originalTopics.topic2,
        log._originalTopics.topic3,
      ]
        .filter(Boolean)
        .map(padTopic32);
    }
    ...
    if (!iface) {
      ...
      if (!parsed && !allowParseFailure)
        throw new Error(
          `Failed to parse log: ${JSON.stringify(log.transactionHash)}`,
        );

      // Enrich args with named properties to match Viem output structure
      if (parsed?.args && event && iface) {
        log.args = enrichEthersArgsWithNamedProperties(
          parsed.args,
          event,
          iface,
        );
