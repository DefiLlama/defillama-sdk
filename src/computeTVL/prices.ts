...
async function makeCoingeckoCall(url: string, coingeckoMaxRetries: number, getCoingeckoLock: GetCoingeckoLog,) {
...
export async function getTokenPrices(
    ...
    for (let i = 0; i < newIds.length; i += 100) {
        const tempTokenPrices = await makeCoingeckoCall(
            `https://api.coingecko.com/api/${url}=${newIds.slice(i, i + 100).join(",")}&vs_currencies=usd`,
            coingeckoMaxRetries,
            getCoingeckoLock)
...
export async function getHistoricalTokenPrices(
    ...
    for (const id of ids) {
        const range = await makeCoingeckoCall(
            `${url}/${id}/market_chart/range?vs_currency=usd&from=${timestamp - secondsPerHalfDay}&to=${timestamp + secondsPerHalfDay}`,
            coingeckoMaxRetries,
            getCoingeckoLock)
