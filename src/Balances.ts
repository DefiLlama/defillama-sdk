...
function getOptions({
  ...
  optionsOrLabel?: BalancesOptionsWithLabel;
...
function isLlamaBalancesObject(o: any): boolean {
  return (
    typeof o === "object" &&
    o !== null &&
    (o as any)._llamaBalancesObject === true
  );
}
...
export class Balances {
  ...
  _add(
    ...
    const {
      ...
      tags = [],
      isUSDValue,
    } = getOptions({ optionsOrLabel, options });
    ...
    if (label) {
      this._breakdownBalances[label] =
        this._breakdownBalances[label] ??
        new Balances({ chain: this.chain, timestamp: this.timestamp });
      this._breakdownBalances[label]._add(token, balance, {
        skipChain,
        isUSDValue,
      });
    }
    ...
    if (tags.length > 0) {
      tags.forEach((tag) => {
        this._taggedBalances[tag] =
          this._taggedBalances[tag] ??
          new Balances({ chain: this.chain, timestamp: this.timestamp });
        this._taggedBalances[tag]._add(token, balance, {
          skipChain,
          isUSDValue,
        });
      });
  ...
  add(
    ...
    if (isLlamaBalancesObject(token)) {
      if (typeof optionsOrLabel === "string")
        throw new Error(
          "When adding a Balances instance, optionsOrLabel must be an object with options, not a string",
        );
      this.addBalances(
        token as Balances,
        balance as BalancesOptionsWithLabel,
        optionsOrLabel as BalancesOptions,
      );
      return;
    }
  ...
  addBalances(
    balances: BalancesV1 | Balances,
    optionsOrLabel?: BalancesOptionsWithLabel,
    options?: BalancesOptions & { skipBreakdown?: boolean },
  ) {
    options = getOptions({ optionsOrLabel, options });
    if (isLlamaBalancesObject(balances)) {
      ...
      if (balancesInstance.hasTaggedBalances()) {
        ...
        const { label, ...restOptions } = options;
        Object.entries(balancesInstance.getTaggedBalances()).forEach(
          ([tag, taggedBalance]) => {
            this._taggedBalances[tag] =
              this._taggedBalances[tag] ??
              new Balances({ chain: this.chain, timestamp: this.timestamp });
      ...
      if (Object.keys(balancesInstance._usdBalances).length > 0) {
        Object.entries(balancesInstance._usdBalances).forEach(
          ([token, balance]) => {
            this._add(token, balance, {
              ...options,
              skipChain: true,
              isUSDValue: true,
            });
  ...
  async getUSDValue() {
    let { usdTvl } = (await computeTVL(
      this.getBalances(),
      this.timestamp,
    )) as any;
    usdTvl = Object.values(this._usdBalances as any).reduce(
      (a, b: any) => a + b,
      usdTvl as number,
    );
  ...
  async getUSDJSONs({
    debug = false,
    debugOptions: { printTokenTable = true, minTokenUSDValue } = {},
  }: {
    debug?: boolean;
    debugOptions?: {
    ...
    let { usdTvl, usdTokenBalances, debugData } = (await computeTVL(
      this.getBalances(),
      this.timestamp,
      { debug },
    )) as any;
    const response = {
      usdTvl,
      usdTokenBalances,
      rawTokenBalances: this.getBalances(),
      labelBreakdown: {},
    };
    ...
    if (debug) {
      ...
      if (minTokenUSDValue)
        tokenData = tokenData.filter((i: any) => +i.value >= minTokenUSDValue!);

      tokenData.sort((a: any, b: any) => b.value - a.value); // sort descending by value

      if (printTokenTable) {
        const dataClone = tokenData.map((i: any) => ({
          ...i,
          value: humanizeNumber(i.value),
        }));
    ...
    if (this.hasBreakdownBalances()) {
      for (const [label, breakdown] of Object.entries(
        this.getBreakdownBalances(),
      )) {
    ...
    if (this.hasTaggedBalances()) {
      ...
      for (const [tag, taggedBalance] of Object.entries(
        this.getTaggedBalances(),
      )) {
  ...
  async debug({
    ...
    return this.getUSDJSONs({
      debug: true,
      debugOptions: { printTokenTable, minTokenUSDValue },
    });
  }
  ...
  subtract(
    ...
    if (isLlamaBalancesObject(balances)) {
      ...
      Object.entries(balancesInstance._usdBalances).forEach(
        ([token, balance]) => {
          this._add(token, Number(balance) * -1, {
            skipChain: true,
            label: options!.label,
            isUSDValue: true,
          });
  ...
  removeNegativeBalances() {
    ...
    Object.keys(this._usdBalances).forEach((token) => {
      if (Number(this._usdBalances[token]) <= 0)
        delete this._usdBalances[token];
    });
  ...
  async _breakdownBalancesAction(action: string, args: any[] = []) {
    if (!this._breakdownBalances) return {};
  ...
  async _taggedBalancesAction(action: string, args: any[] = []) {
    if (!this._taggedBalances) return {};
  ...
  hasBreakdownBalances() {
    return (
      Object.keys(this._breakdownBalances).length > 0 &&
      Object.values(this._breakdownBalances).some((b) => !b.isEmpty())
    );
  }

  hasTaggedBalances() {
    return (
      Object.keys(this._taggedBalances).length > 0 &&
      Object.values(this._taggedBalances).some((b) => !b.isEmpty())
    );
  }

  isEmpty() {
    return (
      Object.keys(this._balances).length === 0 &&
      Object.keys(this._usdBalances).length === 0
    );
  }
