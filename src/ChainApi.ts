...
export class ChainApi {
  ...
  constructor(params: {
    ...
    this.api = this;
    const uKey = this.storedKey
      ? `${params.protocol}:${this.storedKey}`
      : params.protocol
        ? `${params.protocol}`
        : `chain-${this.chain}`;
    const stats = {
  ...
  addTokens(
    tokens: string[],
    balances: any[],
    options: BalancesV2Options = {},
  ) {
    this.add(tokens, balances, options);
  }

  addBalances(
    balances: Balances | BalancesV1,
    options: BalancesV2Options = {},
  ) {
  ...
  async getTokenBalances({
    ...
    if (tokens.length) {
      ...
      tokensAndOwners.push(
        ...(tokens.map((i) => owners.map((j) => [i, j])).flat() as any),
      );
    }
    ...
    this.addStat(
      "sumTokens",
      erc20TokensAndOwners.length + ethBalOwners.length,
      true,
    );

    const bals = await this.multiCall({
      calls: erc20TokensAndOwners.map(([token, owner]: any) => ({
        target: token,
        params: [owner],
      })),
      abi: "erc20:balanceOf",
    ...
    if (ethBalOwners.length) {
      ...
      ).output.map((i: any) => i.balance);

      ethBals.map(
        (i, idx) =>
          (tokenBalances[ethBalanceResponseIndex[idx]] = [nullAddress, i]),
      );
    }
  ...
  async sumTokens({
    ...
    tokenBalances.forEach(([token, balance]) =>
      this.addToken(token, balance, balancesV2Options),
    );
