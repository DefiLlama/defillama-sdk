...
const defaultChunkSize = !!process.env.SDK_MULTICALL_CHUNK_SIZE
  ? +process.env.SDK_MULTICALL_CHUNK_SIZE
  : 300;
...
export async function multiCall(params: MulticallOptions): Promise<any> {
  ...
  if (failedQueries.length) {
    debugLog(
      `[chain: ${params.chain ?? "ethereum"}] [abi: ${
        params.abi
      }] Failed multicalls:`,
      failedQueries.map((r) => r.input),
    );
