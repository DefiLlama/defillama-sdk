...
test("tron: chainApi.getBalances", async () => {
  ...
  const multiRes = await tronApi.getGasTokenBalances({
    owners: [binanceColdWallet],
  });
  const tokenBalancesRes = await tronApi.getTokenBalances({
    owners: [binanceColdWallet, binanceColdWallet, binanceColdWallet],
    tokens: [ETHER_ADDRESS],
    skipDuplicates: true,
  });
  const tokenBalancesDupsRes = await tronApi.getTokenBalances({
    owners: [binanceColdWallet, binanceColdWallet, binanceColdWallet],
    tokens: [ETHER_ADDRESS],
  });
