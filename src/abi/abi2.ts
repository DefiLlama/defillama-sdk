...
export async function call(params: CallOptions): Promise<any> {
  if (params.abis) return callWithAlternateAbis();
  ...
  async function callWithAlternateAbis(): Promise<any> {
    ...
    if (!abis || abis.length === 0)
      throw new Error("No alternate ABIs provided!");
...
export async function multiCall(params: MulticallOptions): Promise<any[]> {
  ...
  if (params.field)
    output.forEach((i: any) => (i.output = i.output[params.field!]));
  ...
  async function multiCallWithAlternateAbis(): Promise<any[]> {
    ...
    if (!permitFailure)
      finalOutput.forEach((data: any) => {
        if (!data?.success)
          throw new Error(
            `All ABI alternatives failed for multiCall! Last error: ${data?.error}`,
          );
      });

    if (params.excludeFailed)
      finalOutput = finalOutput.filter((i: any) => i?.success);
...
export async function fetchList(params: FetchListOptions) {
  ...
  if (excludeFailed && groupedByInput)
    throw new Error(
      "excludeFailed and groupedByInput cannot be used together!",
    );
  if (withMetadata && groupedByInput)
    throw new Error("withMetadata and groupedByInput cannot be used together!");
  ...
  if (!target && !targets)
    throw new Error("Need to provide either target or targets parameter!");
  ...
  if (target) {
    ...
    for (let i = startFrom; i < itemLength; i++)
      calls.push({ target, params: [i] });
  ...
  res.forEach((i: any, idx: number) =>
    groupedByRes[indexToTargetMapping[idx]].push(i),
  );
