...
export async function getBalance(params: {
	...
	const data = await limitRPCCalls(() => {
		if (!balanceCache[params.target])
			balanceCache[params.target] = post(
				{ address: params.target, visible: true },
				"/wallet/getaccount",
			);
	...
	const frozenBalanceV2 =
		data.frozenV2?.reduce((t: any, { amount = 0 }: any) => t + amount, 0) ?? 0;
	const freeBalance = data.balance ?? 0;
	const delegatedBandwidthBalance =
		data.delegated_frozenV2_balance_for_bandwidth ?? 0;
	const delegatedEnergyBalance =
		data.account_resource?.delegated_frozenV2_balance_for_energy ?? 0;
	let balance = (
		freeBalance +
		frozenBalance +
		frozenBalanceV2 +
		delegatedBandwidthBalance +
		delegatedEnergyBalance
	).toString();
...
async function post(body = {}, endpoint = "/wallet/triggerconstantcontract") {
	...
	throw new Error(
		shortenString(
			`All TRON RPCs are not working. Errors: ${errors.map((i) => i.message).join("; ")}`,
			1500,
		),
	);
}
