...
export default async function makeMultiCall(
	...
	if (failedEncodingIndexes.length) {
		const goodCallResponses = await makeMultiCall(
			functionABI,
			goodCalls,
			chain,
			block,
		...
		failedEncodingIndexes.forEach((idx) => {
			responses[idx] = {
				input: { params: calls[idx].params, target: calls[idx].contract },
				success: false,
				output: null,
	...
	async function _call() {
		...
		const { output: returnData } = await call({
			...
			params: [false, contractCalls.map((call: any) => [call.to, call.data])],
		});
