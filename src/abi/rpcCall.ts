...
export async function call(provider: ethers.Provider, data: ethers.TransactionRequest, block?: string | number, chain?: string, options = { retry: true }): Promise<any> {
  ...
  let response
  try {
    response = await provider.call(data)
    onComplete()
  } catch (e) {
    onComplete()
    if (retry)
