...
const getReservesAbi2 =
  "function getReserves() view returns (uint112 _reserve0, uint112 _reserve1)";

const calldata =
  '{"abi":{"constant":true,"inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"allPairs","outputs":[{"internalType":"address","name":"","type":"address"}],"payable":false,"stateMutability":"view","type":"function"},"chain":"avax", "target": "0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10", "calls": []}';
test("large muticall avax", async () => {
  const options = JSON.parse(calldata);
  for (let i = 0; i < 500; i++) options.calls.push({ params: i });
    ...
    output: [
      {
        ...
        success: true,
        output: ["22703861331", "3120786254041482210638", "1668779963"],
      },
      {
        input: {
...
test("Set protocol multicall", async () => {
  ...
  expect(res.output[0].output[0].component).toEqual(
    expectedResponse.output[0].output[0][0],
  );
  expect(res.output[1].output[0].module).toEqual(
    expectedResponse.output[1].output[0][1],
  );
  expect(res.output[2].output[0].unit).toEqual(
    expectedResponse.output[2].output[0][2],
  );
  expect(res.output[3].output[0].positionState).toEqual(
    expectedResponse.output[3].output[0][3],
  );
  expect(res.output[4].output[0].data).toEqual(
    expectedResponse.output[4].output[0][4],
  );
});
